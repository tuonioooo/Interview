...
[Redis多线程模型](https://www.cnblogs.com/volare/p/12283355.html)
...
1：从缓存取不到的数据，在数据库中也没有取到，这时也可以将key-value对写为key-null，缓存有效时间可以设置短点，如30秒（设置太长也会导致正常情况也没法使用）。这样可以有效防止攻击用户反复用同一个ID暴力攻击；

2：对一定不存在的 key 进行过滤。可以把所有的可能存在的 key 放到一个大的 Bitmap 中，查询时通过该 bitmap 过滤。

3：接口层增加校验，如用户鉴权检验，id做基础校验，id&lt;=0的直接拦截
...
[https://www.cnblogs.com/moxiaotao/p/10829799.html](https://www.cnblogs.com/moxiaotao/p/10829799.html)
