...
通过使用 _GET / \_index name/ indices_，我们可以获得集群中存在的索引列表
...
1）node.master节点：它将负责管理集群范畴的变更，例如创建或删除索引，添加节点到集群或从集群删除节点

2）node.data节点：持有数据和倒排索引

3）node.client节点：如果将node.master属性和node.data属性都设置为false，那么该节点就是一个客户端节点，扮演一个负载均衡 的角色，将到来的请求路由到集群中的各个节点
